...
public final class WaniKaniAPI {
    ...
    // MARK: - Get Reviews with Pagination

    public func getReviews(updatedAfter: Date? = nil) async throws -> [Review] {
        ...
        while isFirstRequest || nextURL != nil {
            ...
            if isFirstRequest {
                ...
                currentEndpoint = endpoint(path: "/reviews", queryParameters: queryParams)
            } else if let next = nextURL, let parsed = parseNextURL(next) {
                currentEndpoint = endpoint(path: parsed.path, queryParameters: parsed.queryParams)
            } else {
                break
            ...
            let envelope: CollectionEnvelope<Review> = try await networkClient.request(currentEndpoint)
            allReviews.append(contentsOf: envelope.data)
            nextURL = envelope.pages.nextURL
        }
